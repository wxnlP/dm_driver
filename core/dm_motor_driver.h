...
public:
  ...
  struct CtrlCmd {
    CtrlCmd() = default;
    CtrlCmd(MotorType _vel, MotorType _pos, MotorType _torque,
  ...
  DmMotorDriver(CanFdBusBase& _canfd_bus, uint8_t _motor_id) : canfd_bus_(_canfd_bus), motor_id_(_motor_id) {}
  DmMotorDriver(CanFdBusBase& _canfd_bus, uint8_t _motor_id, MotorType _angle_min_limit, MotorType _angle_max_limit)
      : canfd_bus_(_canfd_bus), 
        motor_id_(_motor_id), 
        angle_min_limit_(_angle_min_limit), 
        angle_max_limit_(_angle_max_limit) {
...
private:
  int MITCtrl(MotorType vel, MotorType pos, MotorType torque, MotorType stiffness, MotorType damping);
